...
import com.yj.banner.xxxbanner.loader.MultiViewLoader
...
class MultiActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val arrayList = Gson().fromJson<List<MultiTourismResponse>>(json, (object : TypeToken<List<MultiTourismResponse>>() {}).type)
        arrayList.forEach {
            if (it.id == 90) {
                it.type = 1
            } else {
        ...
        multi_banner.setImages(arrayList)
                .setDelayTime(4000)
                .setBannerAnimation(Transformer.Alpha)
                .setImageLoader(MultiViewLoader())
                .setOnBannerListener(object : OnBannerListener {
                    override fun onBannerClick(position: Int) {
                        Toast.makeText(this@MultiActivity, position.toString(), Toast.LENGTH_SHORT).show()
                    }
                })
                .start()
    }
