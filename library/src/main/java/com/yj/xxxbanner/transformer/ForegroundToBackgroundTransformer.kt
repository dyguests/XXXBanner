...
class ForegroundToBackgroundTransformer : ABaseTransformer(false) {

    override fun onTransform(view: View, position: Float) {
        val height = view.height.toFloat()
        val width = view.width.toFloat()
        val scale = ABaseTransformer.Companion.min(if (position > 0) 1f else Math.abs(1f + position), 0.5f)
